...
import {STOP_WORDS} from "@constants/ApiConstant";
...
`;
const StopWords = ({currentUser,stopWordData,queryParams}) => {
    const [tableData,setTableData] = useState(stopWordData);
    ...
    const actionComponent = (row) => {
        return (
            <div className="rt-datatable-actions">
                {(currentUser && hasPermissions(currentUser.roles,"STOP_WORDS","EDIT")) ? (
                    <button className="box-button" onClick={() => actionHandler(row,"edit")} data-rt-tooltip="Edit">{editIcon({width:16,height:16})}</button>
                ) : null}
                {(currentUser && hasPermissions(currentUser.roles,"STOP_WORDS","DELETE")) ? (
    ...
    const getData = async(appliedFilters) => {
        try{
            setIsLoading(true);
            const {data} = await axios.get(STOP_WORDS.LISTS,{params: appliedFilters});
    ...
    const actionHandler = (row,type) => {
        if(type == "edit"){
            ...
            sweetAlertHelper({title: "<strong>Confirm</strong>",html: `Are you sure you want to delete this ${row.name}?`,showCancelButton: true}).then((result) => {
    ...
    const deleteRecord = async(rowId) => {
        try{
            ...
            const {data} = await axios.delete(`${STOP_WORDS.DELETE}/${rowId}`);
            if(data.status == API_STATUS.SUCCESS){
    ...
    const handleSubmit = async() => {
        try{
            setSubmitting(true);
            document.getElementById("custom-loader").style.display = "block";
            const {data} = await axios.post(STOP_WORDS.ADDUPDATE,form);
            if(data.status == API_STATUS.SUCCESS){
                document.getElementById("custom-loader").style.display = "none";
                toast.success(data.message,TOAST_OPTIONS);
                setForm({name: ""});
    ...
    return (
        <React.Fragment>
            <Head>
                <title>{`Stop Words - ${APP_NAME}`}</title>
            </Head>
            <BackendLayout page="stop-words">
                {showPopup && (
                    <PopupWrap>
                        <div className="back" onClick={handleClosed}></div>
                        <form onSubmit={onSubmit} className="inner" autoComplete="off">
                            <div className="header">
                                <span className="heading">{form.slug ? "Edit" : "Add"} Stop Word</span>
                ...
                <Wrap>
                    <div className="head-wrap">
                        <CustomHeading>Stop Words</CustomHeading>
                        <div className="action-wrap">
                            {(currentUser && hasPermissions(currentUser.roles,"STOP_WORDS","ADD")) ? (
                                <button className="button" type="button" onClick={() => togglePopup(true)}>Add Stop Word</button>
                            ) : null}
                        </div>
                    </div>
...
export const getServerSideProps = async(context) => {
    let stopWordData = {data: [],current: 1,totalPages: 1},queryParams = {page:1,limit:10,column:"name",dir:"ASC",query:""};
    let token = context.req.cookies[`${APP_SLUG}-token`] || "";
    try{
        const {data} = await axiosApi.get(STOP_WORDS.LISTS,{params: queryParams,headers: {authorization: token}});
        if(data.status == API_STATUS.SUCCESS){
            stopWordData = data.resData;
        }
    }catch(e){
        return handleUnauthorized(e,context.res);
    }
    return getServerProps(context,{stopWordData,queryParams},"STOP_WORDS:LIST");
}
